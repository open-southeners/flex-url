...
- **`strictCommaEncoding` option**, passed as the third argument to `flexUrl()`
  or into `FlexUrlOptions` (PHP): restores the pre-3.0 behaviour opt-in — a
  list value is split on a *raw* `,` only, decoding each piece afterwards
  (instead of decoding first and splitting on whatever commas fall out), so a
