...
2. **v1 → v2: the separator comma stopped being percent-encoded**, matching
   apiable's own idiom and the pagination links it generates.

Only the emitted form changed. **On input, v2 treats a comma as a separator
however it was encoded**, so both older formats parse correctly — apiable splits
the decoded value either way, and Symfony's query normalisation (behind
Laravel's `fullUrl()`) rewrites a raw comma as `%2C` on every round-trip
regardless. A comma inside a single value is therefore not representable in
this grammar, in any of the three versions.
...
| `filter[foo]=bar%2Chello` (v0/v1 multi-value) | `['bar', 'hello']` | yes |
...
So **persisted URLs need no rewrite**: a percent-encoded separator, percent-
encoded brackets and the v2 form all parse to the same thing.

One caveat on versions. Reading a percent-encoded separator as a list arrived
with the comma-handling change, which is unreleased at the time of writing —
**2.1.0 and earlier read `filter[foo]=bar%2Chello` as the single opaque value
`"bar,hello"`**. That matters more than it sounds, because Symfony's query
normalisation (behind Laravel's `fullUrl()`, and so behind every Inertia
response) rewrites a raw comma as `%2C` on every round-trip, so on those
versions a multi-value filter breaks on the *next request*, not just for old
bookmarks. Check the installed version before relying on this row.

One consequence does remain:
...
8. Persisted v0/v1 URLs need no rewrite — see *Data compatibility*. Check only
   for code that relied on v1's AND form (repeated `filter[a]=` keys), which
